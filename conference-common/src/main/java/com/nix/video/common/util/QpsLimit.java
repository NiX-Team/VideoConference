...
public final class QpsLimit {
    private final static Logger LOGGER = LoggerFactory.getLogger(QpsLimit.class);
    ...
    public static void acceptLimit(String sign,long qps,Strategy strategy,Integer wait) {
        ...
        if (checkQps(sign,qps)) {
            switch (strategy) {
                ...
                case noting:break;
                case noting_log:LOGGER.warn("{} :qps to achieve limit . limit {}",Thread.currentThread().getName(),qps);break;
                default:break;
            }
        }
    }
    ...
    private static boolean checkQps(String sign,long qps) {
        if (!WINDOWS.containsKey(sign)) {
            SlidingWindow window = WINDOWS.putIfAbsent(sign,new SlidingWindow(sign,qps));
            if (window == null) {
                return false;
            }
            return window.clickQps();
        }
    ...
    private static class SlidingWindow {
        ...
        public boolean equals(Object obj) {
            return sign.equalsIgnoreCase(((SlidingWindow)obj).sign);
        }
    }
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    LOGGER.warn("guardian thread clear windows exception ",e);
                }
                try {
                    sleep(1000 * 60 * 10);
                } catch (InterruptedException e) {
                    LOGGER.error("guardian thread clear windows interrupted . stop!!!");
                    return;
                }
            }
        }
